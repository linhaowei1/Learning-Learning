...
augSyn = naw.SynonymAug(aug_src='wordnet', aug_p = 0.2)
augSwap = naw.RandomWordAug(action = 'swap', aug_p = 0.2) # 90%的swap概率
augDel = naw.RandomWordAug(action = 'delete') # 没有指定变换概率，默认为30%
augMask = naw.RandomWordAug(action = 'substitute', target_words = ['<mask>'], aug_p = 0.2) # 没有指定变换概率，默认为30%
